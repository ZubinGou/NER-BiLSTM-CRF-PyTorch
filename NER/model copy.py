...
def log_sum_exp(vec):
    ...
    return (vec - max_score).exp().sum(axis=0, keepdim=True).log() + max_score
...
class BiLSTM_CRF(nn.Module):
    ...
    def _get_lstm_features(self, sentence, chars, caps, chars2_length, d):
        ...
        if self.char_mode == 'LSTM':
            ...
            chars_embeds_temp = Variable(torch.FloatTensor(torch.zeros((outputs.size(0), outputs.size(2))))).to(self.device)
    ...
    def viterbi_decode(self, feats):
        ...
        best_tag_id = torch.argmax(terminal_var.unsqueeze(0), dim=1)
