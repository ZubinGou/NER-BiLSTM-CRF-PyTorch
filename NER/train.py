...
optparser.add_option(
    "-T", "--train", default="data/eng.train", help="Train set location"
)
...
optparser.add_option(
    "--score", default="evaluation/temp/score.txt", help="score file location"
)
optparser.add_option(
    "-s", "--tag_scheme", default="iobes", help="Tagging scheme (IOB or IOBES)"
)
optparser.add_option(
    "-l",
    "--lower",
...
optparser.add_option(
    "-z", "--zeros", default="0", type="int", help="Replace digits with 0"
)
optparser.add_option(
    "-c", "--char_dim", default="25", type="int", help="Char embedding dimension"
)
optparser.add_option(
    "-C",
    "--char_lstm_dim",
...
optparser.add_option(
    "-w", "--word_dim", default="100", type="int", help="Token embedding dimension"
)
optparser.add_option(
    "-W",
    "--word_lstm_dim",
...
optparser.add_option(
    "-A", "--all_emb", default="1", type="int", help="Load all embeddings"
)
optparser.add_option(
    "-a",
    "--cap_dim",
...
optparser.add_option(
    "-f", "--crf", default="1", type="int", help="Use CRF (0 to disable)"
)
optparser.add_option(
    "-D",
    "--dropout",
...
optparser.add_option(
    "-r", "--reload", default="0", type="int", help="Reload the last saved model"
)
optparser.add_option(
    "-g", "--use_gpu", default="1", type="int", help="whether or not to ues gpu"
)
optparser.add_option("--loss", default="loss.txt", help="loss file location")
optparser.add_option("--name", default="test", help="model name")
optparser.add_option(
    "--char_mode", choices=["CNN", "LSTM"], default="CNN", help="char_CNN or char_LSTM"
)
...
print(
    "%i / %i / %i sentences in train / dev / test."
...
word_embeds = np.random.uniform(
    -np.sqrt(0.06), np.sqrt(0.06), (len(word_to_id), opts.word_dim)
)
...
def evaluating(model, datas, best_F):
    ...
    for data in datas:
        ...
        if use_gpu:
            val, out = model(
                dwords.cuda(), chars_mask.cuda(), dcaps.cuda(), chars_length, d
            )
        else:
    ...
    print(
        classification_report(
            gold_tag_id, pred_tag_id, labels=tag_ids, target_names=tags
        )
    )
...
for epoch in range(1, 10001):
    for iter, index in enumerate(tqdm(np.random.permutation(len(train_data)))):
        ...
        if use_gpu:
            ...
            neg_log_likelihood = model.neg_log_likelihood(
                sentence_in, targets, chars_mask, caps, chars_length, d
            )
        ...
        if count % plot_every == 0:
            ...
            vis.line(
                ...
                win=losswin,
                opts={"title": losswin, "legend": ["loss"]},
            )
        ...
        if (
            count % (eval_every) == 0
            and count > (eval_every * 20)
            or count % (eval_every * 4) == 0
            and count < (eval_every * 20)
        ):
            model.train(False)
            best_train_F, new_train_F, _ = evaluating(
                model, test_train_data, best_train_F
            )
